...
- **Obvious / task-reaffirming comments.** In the same pass, remove comments that only restate what the code plainly does or narrate that the task I asked for is done ("Add the export button", "Now stream the rows", "Debounce the search", "Loop over the items"). These reaffirm completing the work I described instead of explaining anything; they are noise. Default to no comment. Keep one only when it explains something a reader could not get from the code itself and must know to change the line safely: an invariant, a workaround, a surprising default, a non-trivial optimization, or tricky math. Test: if the comment would read as true pasted next to almost any similar code, cut it.
