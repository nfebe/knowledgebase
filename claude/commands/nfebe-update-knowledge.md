...
- Append to the most relevant existing file rather than creating a near-duplicate; create a new file only when no existing one fits. Match the surrounding note style. Follow my writing rules: avoid em dashes (use one only when genuinely clearest), no en-dash punctuation, no `--` in rendered prose, and never use the acronyms "DAG" or "DoD" (spell them out).
